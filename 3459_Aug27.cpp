...
class Solution {
public:
    int solve(vector<vector<vector<vector<int>>>>& dp, int i, int j, int dir,
              int allowed, vector<vector<int>>& grid, int target) {
        ...
        int noChange = 1 + solve(dp, i + dx[dir], j + dy[dir], dir, allowed,
                                 grid, 2 - target);
